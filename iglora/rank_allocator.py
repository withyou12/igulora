...
class RankAllocator(object):
    ...
    def get_lora_param_name(self):
        ...
        for n,p in self.model.named_parameters():
            if "lora_A" in n: 
                ...
                if module_idx > 3:
                    self.total_rank += p.size(0) * (float(os.getenv("DIM_MULTIPLIER")) + 1) / 2
                else:
    ...
    def update_ipt(self, model): 
        for n,p in model.named_parameters():
            if "lora_" in n and "mask" not in n:
    ...
    def calculate_score(self, n, p=None, metric="ipt"):
        if metric == "ipt":
            ...
            ipt_score = (self.exp_avg_ipt[n] + 1e-6) / (self.exp_avg_unc[n] + 1e-6)
            # ipt_score = (torch.sqrt(self.exp_avg_unc[n]) + 1e-6) / (self.exp_avg_ipt[n] + 1e-6)
    ...
    def mask_to_target_rank(self, model, curr_rank): 
        ...
        for name_mat in combine_dict: 
            ...
            layer_idx = int(name_mat.split(".")[2])
            module_name = name_mat.split(".")[4]
            module_idx = {
                "q_proj": 0,
                "k_proj": 1,
            ...
            for rank_idx in range(sum_ipt.view(-1).shape[0]):
                list_rank2scores.append(
                    ((layer_idx, module_idx, rank_idx), sum_ipt.view(-1)[rank_idx])
                )
        ...
        mask_threshold = 0.0
        for (layer_idx, module_idx, rank_idx), score in list_rank2scores:
            if module_idx > 3:
                tmp_rank_sum += (float(os.getenv("DIM_MULTIPLIER")) + 1) / 2
            else:
                tmp_rank_sum += 1

            if tmp_rank_sum >= self.total_rank - curr_rank:
                mask_threshold = score
                break
        ...
        with torch.no_grad():
            ...
            for n,p in model.named_parameters():
                if "lora_E" in n:
                    ...
                    ranks_info = (is_dict[n]>mask_threshold).squeeze().float().detach().cpu().numpy().tolist()
                    ...
                    if self.tb_writter is not None and self.global_step%self.log_interval==0:
                        ...
                        self.rank_pattern[n] = ranks_info
            ...
            if self.tb_writter is not None and self.global_step%self.log_interval==0:
                ...
                self.tb_writter.add_scalar("Budget/mask_threshold", mask_threshold.to(torch.float32).cpu().numpy().tolist(), self.global_step)
    ...
    def update_and_mask(self, model, global_step):
        ...
        # print("total_rank: ", self.total_rank)
        ...
        if mask_ind:
            ...
            # rank_pattern_ = self.get_rank_pattern()
            # print("rank_pattern_: ", rank_pattern_)
